...
while isempty(i_notes)
  ...
  i_notes=mps_tree_get_cond_notes(ST,d_cond(use_cond)); % 2022.8.17 
  % 2022.8.17 new, but in the newest mGstat on github，the 'cat' added in fact
  % i_notes=mps_tree_get_cond_notes(ST,d_cond(use_cond),cat); 
  % So if use the new, must delte the 'mps_tree_get_cond_notes' in the
  % 'relycode', then will use the 'mps_tree_get_cond_notes' in the 'mGstat'
  if isempty(i_notes)

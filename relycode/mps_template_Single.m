...
if method==1
    ...
    % 2022.8.18 add
    squence = {'ascend','descend'};
%     c = 1;
%     for i = 1:size(squence,2)
%         for j = 1:size(squence,2)
%             sort_data{c}=sortrows([d(:) xx(:) yy(:) zz(:)],[1,2,3],{'ascend',squence{i},squence{j}});
%             c = c+1;
%         end
%     end
    % 2021.3.13 change the traversal order of the template
    sort_data=sortrows([d(:) xx(:) yy(:) zz(:)],1);
    % sort_data1=sortrows([d(:) xx(:) yy(:) zz(:)],[1,2,3],{'ascend','ascend','ascend'});
    % sort_data2=sortrows([d(:) xx(:) yy(:) zz(:)],[1,2,3],{'ascend','ascend','descend'});
    % sort_data3=sortrows([d(:) xx(:) yy(:) zz(:)],[1,2,3],{'ascend','descend','ascend'});
    % sort_data4=sortrows([d(:) xx(:) yy(:) zz(:)],[1,2,3],{'ascend','descend','descend'});
    ...
    % 2021.3.13 Modify 
    % template{1}=[sort_data1(2:(n_max+1),2:end)];
    % template{2}=[sort_data2(2:(n_max+1),2:end)];
    % template{3}=[sort_data3(2:(n_max+1),2:end)];
    % template{4}=[sort_data4(2:(n_max+1),2:end)];
    
    % 2022.8.18
%     for i = 1:size(sort_data,2)
%        template{i}=[sort_data{i}(2:(n_max+1),2:end)]; 
%     end    
    
    dist=sort_data(2:(n_max+1),1);
    % 2021.3.13 Modify 
    % dist=sort_data1(2:(n_max+1),1);
    % 2022.8.18 
%     dist=sort_data{1}(2:(n_max+1),1);
...
elseif (method==2)|(method==3)
    ...
    for i_dim=1:n_dim
        for is=[-1 1];
            ii =  [1:n_use].*is;
            % t=zeros(n_use,3);  % 2021.3.10 Modify 
            t=zeros(n_use(1),3);  
            t(:,i_dim)=ii;
            template=[template;t];
        end
